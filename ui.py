...
class SettingsWindow(ctk.CTkToplevel):
    # DİKKAT: on_save_callback parametresine openai_key de eklendi
    def __init__(self, master, current_keys, on_save_callback):
        super().__init__(master)
        self.title("Sirius Ayarlar")
        self.geometry("400x290") # 4 kutu sığsın diye pencereyi uzattık
        ...
        self.key_entry.insert(0, current_keys.get("groq_api_key", "")) 
        ...
        # YENİ: OpenAI Kutusu Eklendi!
        self.openai_entry = ctk.CTkEntry(self, width=300, placeholder_text="OpenAI API Key (sk-...)", fg_color="#0a0a0a", border_color="#ffffff", text_color="white")
        self.openai_entry.pack(pady=5)
        self.openai_entry.insert(0, current_keys.get("openai_api_key", ""))
    ...
    def save_and_close(self):
        # YENİ: 4. parametre olarak openai_entry de gönderiliyor
        self.on_save_callback(self.key_entry.get().strip(), self.serp_entry.get().strip(), self.gemini_entry.get().strip(), self.openai_entry.get().strip())
        self.destroy()
